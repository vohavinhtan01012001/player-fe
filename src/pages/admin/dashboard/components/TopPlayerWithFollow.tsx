...
export default function TopPlayerWithFollow() {
    const [selectedYear, setSelectedYear] = useState(getCurrentYear());
    const [selectedMonth, setSelectedMonth] = useState(getCurrentMonth());
    const [data, setData] = useState<any[]>([])
    const getTopPlayer = async () => {
        try {
            const res = await FollowerService.getTopPlayer(selectedMonth, selectedYear);
    ...
    return (
        <div className="flex flex-col py-6 justify-center items-center w-full bg-white shadow-lg rounded-lg px-4 border">
            <h1 className="font-semibold text-2xl tracking-wider uppercase text-center">Top 5 Most Follower Players</h1>
            <div className='py-2'>
